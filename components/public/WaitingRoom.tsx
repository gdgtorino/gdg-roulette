...
export function WaitingRoom({ eventId }: WaitingRoomProps) {
  ...
  const { isConnected, getLatestEvent } = useEventSSE(eventId || '');
    ...
    if (lastEvent) {
      switch (lastEvent.type) {
        ...
        case 'participant_left':
          setParticipants((prev) => prev.filter((p) => p.id !== lastEvent.data.participantId));
          break;
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Waiting for Draw to Begin</h2>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
          <span className="text-lg font-semibold text-blue-900">Current Participants</span>
        ...
        {eventStatus === 'drawing' && (
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <LoadingSpinner size="sm" className="mb-2" />
            <p className="text-green-800">Draw is starting! Please wait...</p>
          </div>
        )}
      </div>

      <div className="mt-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-3">Recent Participants</h3>
