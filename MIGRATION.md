...
This document outlines the migration of the Express backend to Next.js API
routes for The Draw lottery system.

## Migration Overview

The Express backend has been successfully migrated to Next.js App Router API
routes with the following improvements:
...
- `GET /api/events/[eventId]/participants/[participantId]/winner` - Check if
  winner
...
All existing API endpoints remain functional, ensuring seamless migration for
existing clients. The Express backend can be gradually deprecated as clients
migrate to the new Next.js API.
